...
class FirebaseShareService(
    ...
    override suspend fun uploadConfig(jsonString: String): String? = withContext(Dispatchers.IO) {
        try {
            ...
            val userId = SupabaseManager.currentUser.value ?: "anonymous"
    ...
    override suspend fun listConfigs(): List<CloudService.CloudConfig> = withContext(Dispatchers.IO) {
        try {
            val userId = SupabaseManager.currentUser.value ?: "anonymous"
            // Firebase 列表过滤通常需要索引，这里使用简单的查询参数或在客户端过滤
            // 为了简单起见，且 Firebase 默认 list 不支持复杂过滤，我们通过构建结构化查询或在 URL 中尝试（取决于具体配置）
            // 这里我们采用最通用的做法：在 URL 中添加简单过滤（若 Firestore 规则允许）
            ...
            for (i in 0 until documents.length()) {
                ...
                // 客户端过滤：只保留属于当前用户的内容
                val itemUserId = fields.optJSONObject("userId")?.optString("stringValue") ?: "anonymous"
                ...
                result.add(CloudService.CloudConfig(shareCode, createdAt ?: 0, downloadCount, preview, type, true))
            }
